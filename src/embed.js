...
export async function embed(texts) {
  if (CONFIG.EMBED_PROVIDER === "server") {
    // Hard guard: index.js must skip embed() entirely when EMBED_PROVIDER=server.
    // If we reach here, the caller has a bug — fail loudly instead of silently
  ...
  if (CONFIG.EMBED_PROVIDER === "lovable") return embedLovable(texts);
  if (CONFIG.EMBED_PROVIDER === "openai") return embedOpenAI(texts);
  if (CONFIG.EMBED_PROVIDER === "mock") {
    // The old FNV-style mock collapsed to a constant seed for any input
    // longer than a few hundred chars, so every long chunk received the SAME
    // vector [-0.04418591…, 0.03982766…, …]. That poisoned every embedding in
    // the DB and broke retrieval. Mock is permanently disabled — never
    // reintroduce it. Use EMBED_PROVIDER=server (default) in production.
    throw new Error(
...
async function embedLovable(texts) {
  ...
  const out = [];
  const BATCH = 100; // Gateway cap for google/gemini-embedding-001
